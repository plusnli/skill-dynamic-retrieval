...
class Evaluator:
    ...
    def __call__(self, info, client="gpt-3.5", version="naive"):
        ...
        if self.log_save_path:
            ...
            with open(f"{self.log_save_path}/{info['traj_name']}.md", "w") as md_file:
                ...
                if info.get("final_state"):
                    md_file.write(f"## Final State (axtree)\n\n```md\n{info['final_state']}\n```\n\n")
    ...
    def eval_text(self, info, client):
        ...
        # Judge context.
        last_think = info["captions"][-1] if info.get("captions") else ""
        if info.get("eval_source") == "think":
            page_info = last_think
        else:
            page_info = info.get("final_state") or ""
            if not page_info.strip():
                page_info = last_think
        prompt, sys_msg = build_text_eval_prompt(
            page_info, info["intent"], response, action_history
        )
